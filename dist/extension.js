...
var BaseAnthropic = class {
  ...
  getAPIList(path5, Page3, opts) {
    return this.requestAPIList(Page3, { method: "get", path: path5, ...opts });
  }
...
var AnthropicProvider = class {
  ...
  async generateTest(request) {
    ...
    const content = message.content.map((block) => block.type === "text" ? block.text : "").filter(Boolean).join("\n");
    if (!content) {
...
function getWorkspaceRoot(uri) {
  ...
  return path3.dirname(uri.fsPath);
}
function getInitialTestFilePath(input) {
  const testRoot = path3.isAbsolute(input.testDir) ? path3.normalize(input.testDir) : path3.resolve(input.workspaceRoot, input.testDir);
  const mirroredParts = getMirroredDirectoryParts(input.relativeSourcePath);
  const targetDirectory = path3.join(testRoot, ...mirroredParts);
  return path3.join(targetDirectory, `${input.testBaseName}.test.ts`);
}
function computeRelativeImportPath(testFilePath, sourceFilePath) {
  const fromDirectory = path3.dirname(testFilePath);
  const sourceWithoutExtension = stripSourceExtension(sourceFilePath);
  let relativeImport = normalizeToPosix(path3.relative(fromDirectory, sourceWithoutExtension));
  if (!relativeImport.startsWith(".")) {
    relativeImport = `./${relativeImport}`;
  }
  return relativeImport;
}
async function writeNewFileWithSuffix(initialFilePath, content) {
  const parsed = path3.parse(initialFilePath);
  await fs.mkdir(parsed.dir, { recursive: true });
  for (let index = 0; index < 1e3; index += 1) {
    const filePath = index === 0 ? initialFilePath : path3.join(parsed.dir, `${parsed.name}.${index}${parsed.ext}`);
    try {
...
function getMirroredDirectoryParts(relativeSourcePath) {
  const parts = normalizeToPosix(relativeSourcePath).split("/").filter((part) => part && part !== "." && part !== "..");
...
function detectSymbolName(selectedCode) {
  const patterns = [
...
function detectTypesInSelection(selectedCode) {
  const declarations = [
...
function isReactLikeSelection(languageId, selectedCode) {
  if (languageId === "javascriptreact" || languageId === "typescriptreact") {
    return true;
  }
  return /<[A-Z][A-Za-z0-9.]*[\s>/]/.test(selectedCode) || /\bReact\./.test(selectedCode) || /\buse[A-Z][A-Za-z0-9_]*\s*\(/.test(selectedCode);
}
function matchDeclarations(source, pattern) {
  return Array.from(source.matchAll(pattern), (match) => match[0].trim());
}
function containsOnlyWhitespaceOrComments(value) {
...
// src/services/testGenerator.ts
var SYSTEM_PROMPT = `You are an expert TypeScript test engineer. You write production-quality Vitest tests.
...
async function generateVitestTestFromSelection(editor) {
  ...
  try {
    const timeoutPromise = new Promise(
      (_, reject) => setTimeout(
        () => reject(
          new Error(
            `LLM request timed out after ${LLM_TIMEOUT_MS / 1e3}s. Check your API key, network, or try a faster model.`
          )
        ),
        LLM_TIMEOUT_MS
      )
...
function getSettings() {
  const config = vscode2.workspace.getConfiguration("vitestGenerator");
  const providerValue = config.get("provider", "openai");
  const provider = isProviderName(providerValue) ? providerValue : "openai";
  const modelOverride = config.get("model", "").trim();
  return {
    testDir: config.get("testDir", "./test").trim() || "./test",
    provider,
    apiKey: config.get("apiKey", "").trim(),
    model: modelOverride || DEFAULT_MODELS[provider]
  };
}
function buildGenerationContext(editor, testDir, selectedCode) {
  const document = editor.document;
  ...
  const relativeSourcePath = normalizeToPosix(
    path4.relative(workspaceRoot, sourceFilePath)
  );
...
function ensureModuleImportPath(testCode, importPath, sourceFilePath, detectedName, selectedCode) {
  if (hasImportFromPath(testCode, importPath)) {
    return testCode;
  }
  const sourceStem = path4.basename(sourceFilePath).replace(/\.[^.]+$/, "");
  ...
  const importStatement = !detectedName ? `import * as moduleUnderTest from '${importPath}';` : isDefaultExport(selectedCode) ? `import ${detectedName} from '${importPath}';` : `import { ${detectedName} } from '${importPath}';`;
  return `${importStatement}
${testCode}`;
}
function hasImportFromPath(testCode, importPath) {
  const importPathPattern = new RegExp(
    `from\\s+['"]${escapeRegExp(importPath)}['"]`
  );
  return importPathPattern.test(testCode);
}
function isDefaultExport(selectedCode) {
...
function activate(context) {
  const command = vscode3.commands.registerCommand("vitestGenerator.generateVitestTest", async () => {
    const editor = vscode3.window.activeTextEditor;
    if (!editor) {
      vscode3.window.showErrorMessage("Open a JavaScript or TypeScript file and select code to generate a Vitest test.");
      return;
    }
    if (editor.selection.isEmpty) {
      vscode3.window.showErrorMessage("Select code before running Generate Vitest Test.");
      return;
    }
    try {
      ...
      const action = await vscode3.window.showInformationMessage(
        `Vitest test generated: ${vscode3.workspace.asRelativePath(result.testFilePath)}`,
        openAction
      );
      if (action === openAction) {
        const document = await vscode3.workspace.openTextDocument(vscode3.Uri.file(result.testFilePath));
        await vscode3.window.showTextDocument(document);
      }
    } catch (error) {
      if (error instanceof MissingApiKeyError) {
        vscode3.window.showErrorMessage("vitestGenerator.apiKey is not set. Please add it to your settings.");
        return;
      }
      ...
      if (error instanceof FileWriteError) {
        vscode3.window.showErrorMessage(`Failed to write ${error.filePath}: ${error.message}`);
        return;
      }
      vscode3.window.showErrorMessage(error instanceof Error ? error.message : String(error));
    }
  });
